...
SWIFT_CLASS("_TtC20VeridiumOrchestrator13CountdownView")
...
@interface OfflineModeViewController (SWIFT_EXTENSION(VeridiumOrchestrator)) <BaseViewProtocol>
@end
...
@property (nonatomic, copy) NSString * _Nullable automaticQRAuthenticationProfile;
...
- (VIDServerConfiguration * _Nullable)getServerConfigurationsForProfileId:(NSString * _Nonnull)profileId SWIFT_WARN_UNUSED_RESULT;
- (NSDictionary<NSString *, VIDServerConfiguration *> * _Nonnull)getAllServerConfigurations SWIFT_WARN_UNUSED_RESULT;
...
/// \param pushTokenData token received on application didRegisterForRemoteNotificationsWithDeviceToken
///
///
/// returns:
/// void
- (void)setRemoteNotificationsTokenWithPushTokenData:(NSData * _Nullable)pushTokenData;
/// Fetches notifications - if any - from the each paired environment
/// \param completion callback with error parameter. Provides error if any error happens during fetching notifications. Provides nil otherwise
///
- (void)refreshNotificationsWithCompletion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Consumes pending pushes from all paired environments
- (void)consumePendingPushes;
/// Checks if there is any push authentication in progress for all paired environments
///
/// returns:
/// <code>true</code>, if there is any push authentication in progress. <code>false</code>, otherwise.
- (BOOL)hasAPushAuthenticationInProgress SWIFT_WARN_UNUSED_RESULT;
...
/// returns:
/// void
- (void)startInMotionSessionWithProfile:(VIDProfile * _Nonnull)profile event:(NSString * _Nonnull)event;
/// Record UBA event
...
/// returns:
/// void
- (void)recordInMotionEventWithProfile:(VIDProfile * _Nonnull)profile event:(NSString * _Nonnull)event;
/// Stop UBA event
/// Stop UBA session.
///
/// returns:
/// void
- (void)stopUBASessionWithProfile:(VIDProfile * _Nonnull)profile withSuccess:(BOOL)withSuccess;
...
- (void)wipeAccountsFromAllEnvironmentsWhenDone:(void (^ _Nullable)(void))whenDone;
/// Returns an array including <code>HelpContent</code> instances for each paired environment
/// If there is not a paired environment available yet, returns an array contatining one default help content instance
...
- (NSArray<HelpContent *> * _Nonnull)getHelpContents SWIFT_WARN_UNUSED_RESULT;
@end
...
SWIFT_CLASS("_TtC20VeridiumOrchestrator22VIDServerConfiguration")
@interface VIDServerConfiguration : NSObject
@property (nonatomic, readonly, copy) NSString * _Nonnull demoBankURL;
@property (nonatomic, readonly, copy) NSArray<NSString *> * _Nonnull supportEmailAddresses;
@property (nonatomic, readonly, copy) NSDictionary<NSString *, NSString *> * _Nonnull userAgentToUriSchemeMap;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_DEPRECATED_MSG("-init is unavailable");
@end
...
@interface VeridiumRegisterViewController : UIViewController <UIAdaptivePresentationControllerDelegate>
- (void)viewDidLoad;
- (void)viewDidAppear:(BOOL)animated;
- (void)viewDidLayoutSubviews;
- (void)viewWillAppear:(BOOL)animated;
...
@interface VeridiumRegisterViewController (SWIFT_EXTENSION(VeridiumOrchestrator)) <BaseViewProtocol>
@end
